<?php
function gavias_facdori_font_size(){
    $size = array();
    $size[''] = '-- Default --';
    for ($i=12; $i <= 50 ; $i = $i+1) { 
        $size[$i] = $i;
...
function gavias_facdori_render_option_font(){
    $fonts = gavias_facdori_fonts();
    $output = '';
...
function gavias_facdori_typography_font_styles($option, $selectors) {
    $output = $selectors . ' {';
...
function gavias_facdori_typography_enqueue_google_font($font) {
 
    if($font && $font != "---"){
        if(array_search($font, array_keys(gavias_facdori_fonts())) > 17){
...
function gavias_facdori_links_typography_font($json){
    $links_fonts = '';
    $customize = (array)json_decode($json, true);
    if(isset($customize['font_family_primary']) && $customize['font_family_primary']){
        $links_fonts .= gavias_facdori_typography_enqueue_google_font($customize['font_family_primary']);
    }
    if(isset($customize['font_family_second']) && $customize['font_family_second']){
        $links_fonts .= gavias_facdori_typography_enqueue_google_font($customize['font_family_second']);
    }
    return $links_fonts;
}

function gavias_facdori_options_patterns(){
    $output = '';
    $file_path = drupal_get_path('theme', 'gavias_facdori');
